...
Generated by TubeClear AI • ${new Date().toLocaleString()}
  ...
  const executePreScanOnly = useCallback(async (
    ...
    try {
      console.log('🔍 Starting pre-scan for:', input.platformId, input.title);
      
      // STAGE 1: Scrape metadata
      const metadata = scrapeMetadata(input);
      console.log('✅ Metadata scraped:', { title: metadata.title, tags: metadata.tags.length });
      
      // STAGE 2: Match against live policies
      const patternResult = matchLivePatterns(metadata, input.platformId);
      console.log('✅ Pattern matching done:', { riskScore: patternResult.riskScore, cleanStatus: patternResult.cleanStatus });
      ...
      return {
        ...
        issues: [],
        riskScore: 0,
        requiresDeepScan: true, // Always recommend deep scan on error
      };
    }
